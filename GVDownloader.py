...
gvd_label = ttk.Label(labels_frame,
                      text='GVDownloader -version 0.0.1!')
gvd_label.pack(pady=1, fill=X, side=TOP)
info_label = ttk.Label(labels_frame,
                       text='Download videos, playlist, streams from many resources!')
...
path_button = ttk.Button(path_frame,
                         text="выбрать каталог для загрузки", width=32,
                         command=get_folder)
...
url_button = ttk.Button(urls_frame,
                        text='Вставьте ссылку из буфера обмена', width=32,
                        command=get_url)
...
videoservice_button = ttk.Button(videoservice_frame,
                             text='Посмотреть список видеосервисов',
                             command=open_video_source)
...
download_button = ttk.Button(download_frame,
                             text='Скачать видео. плейлист или живой стрим',
                             command=video_load)
...
exit_button = ttk.Button(footer_frame,
...
l = ttk.Style()
l.configure('TLabel', font=('Arial', 14, 'bold'), padding=2, foreground='#A52A2A')
e = ttk.Style()
